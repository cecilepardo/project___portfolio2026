...
const ThemeToggle: React.FC<ThemeToggleProps> = ({ theme, toggleTheme, className }) => {
  return (
    <button 
      type="button"
      onClick={toggleTheme}
      className={`flex items-center justify-center rounded-xl bg-[var(--color-nav)] shadow-lg border border-white/10 transition-transform hover:scale-110 cursor-pointer ${className}`}
      aria-label="Changer de thème"
    >
      {theme === 'dark' ? (
        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 fill-amber-400" viewBox="0 0 24 24">
          <title>Activer le mode clair</title>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 fill-slate-700" viewBox="0 0 24 24">
          <title>Activer le mode sombre</title>
